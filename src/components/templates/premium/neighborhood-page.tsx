...
interface PremiumNeighborhoodPageProps {
  data: {
...
export function PremiumNeighborhoodPage({ data, siteSlug, locationSlug }: PremiumNeighborhoodPageProps) {
  ...
  const breadcrumb = buildBreadcrumbSchema([
    ...
    { name: neighborhood.name, url: paths.neighborhoodPage(neighborhood.slug, locationSlug) },
  ]);

  return (
    <PremiumShell formOnPage={false} site={site} primaryLocation={location} siteSlug={siteSlug} locationSlug={locationSlug} ctaStyle="booking">
      <JsonLd data={[breadcrumb]} />
      <PremiumPageHero
        crumbs={[{ label: 'Home', href: paths.locationHome(locationSlug) }, { label: neighborhood.name }]}
